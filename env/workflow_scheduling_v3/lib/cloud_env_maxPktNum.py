...
class cloud_simulator(object):
    def __init__(self, args):
        ...
        if self.set.is_wf_trace_record:
            ...
            self.pkt_trace_file = os.path.join(
                __location__, r'allocation_trace_%s_seed%s_arr%s_gamma%s.csv' %
                (args["algo"],  args["seed"], args["arrival rate"], args["gamma"]))
            write_csv_header(self.pkt_trace_file, [
                'Workflow ID', 'Workflow Pattern', 'Workflow Arrival Time', 'Workflow Finish Time', 'Workflow Deadline', 'Workflow Deadline Penalty',
                                                   'Task Index', 'Task Size', 'Task Execution Time', 'Task Ready Time', 'Task Start Time', 'Task Finish Time',
                                                   'VM ID', 'VM speed', 'Price', 'VM Rent Start Time', 'VM Rent End Time', 'VM Pending Index'])
    ...
    def workflow_generator(self, usr, appID, index):
        ...
        pkt = Workflow(self.usrcurrentTime[usr], wrf, appID, usr, self.set.dataset.wsetSlowestT[appID], self.set.dueTimeCoef[usr, appID], self.wrfIndex)
        self.usr_queues[usr].enqueue(pkt, self.usrcurrentTime[usr], None, usr, 0)
    ...
    def step(self, action):
        # Map the action and dispatch the task.
    ...
    def state_info_construct(self):
        ...
        for vm_ind in range(len(self.vm_queues)):
            ...
            if task_est_finishTime + self.nextTimeStep < self.appSubDeadline[self.nextWrf][self.nextTask]:
                ...
                meetDeadline = 0
                extraCost += 1 + self.set.dataset.wsetBeta[self.nextWrf.get_appID()] * (
                    task_exe_time + self.nextTimeStep - self.appSubDeadline[self.nextWrf][self.nextTask])
            ob.append([])
        ...
        for dcind in range(self.dcNum):
            for cpuNum in self.set.dataset.vmVCPU:
                ...
                VM_features = [cpuNum, self.nextTimeStep, self.nextTimeStep+extra_VM_hour, 0, 0, 0]
                VM_features_matrix.append([])
                VM_features_matrix[-1] = VM_features
        ...
        return ob, self.nextWrf.app, self.nextTask, VM_configuration
