...
class SelfAttentionEncoder(nn.Module):
    def __init__(self, task_fea_size, vm_fea_size, output_size, d_model, att_heads, att_en_layers, d_ff, gat_heads, dropout=0.1):
        super(SelfAttentionEncoder, self).__init__()

        self.task_feature_enhance = nn.Sequential(nn.Linear(task_fea_size, 128), nn.ReLU(), nn.Linear(128, 128), nn.ReLU(), nn.Linear(128, d_model))
        ...
        self.priority = nn.Sequential(nn.Linear(6 * d_model, 128), nn.ReLU(), nn.Linear(128, 128), nn.ReLU(), nn.Linear(128, output_size))
